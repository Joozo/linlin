					...
					$(".lay-cal .tocal").click(function(){
						...
						switch(type){
							...
							case "4"://等额本息
								...
								$(".lay-cal .tal:eq(2)").addClass("cur").html("每月应收本息：<span>"+milFt.contUn(Number(month).toFixed(2))+"</span>元");
							break;
							case "2"://先息后本
								...
								$(".lay-cal .tal:eq(2)").addClass("cur").html("每月应收利息：<span>"+milFt.contUn(Number(month).toFixed(2))+"</span>元");
								$(".lay-cal .tal:eq(3)").addClass("cur").find("span").html(milFt.contUn(Number(last).toFixed(2)));
							break;
						}
						$(".lay-cal .tal:eq(0)").addClass("cur").find("span").html(milFt.contUn(Number(total)));
						$(".lay-cal .tal:eq(1)").addClass("cur").find("span").html(milFt.contUn(Number(interest)));
					});
